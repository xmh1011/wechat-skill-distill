...
- 浏览器只调用本地 `/api/chat`，API key 只在 server-side proxy 使用；浏览器不得接收模型 base URL；默认不得接受浏览器传入的 model 覆盖，除非服务端显式开启 override；默认不得向浏览器返回 provider/memory 原始错误细节，除非服务端显式开启 debug errors。
