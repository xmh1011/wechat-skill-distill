...
- 基础指标：消息数、平均长度、短句比例、问句占比、多行消息占比、表达节奏、表情/符号倾向、常见表达、样本句。
...
- 模型请求前必须过滤明确属于其他 userID 的 memory hits；过滤只基于 metadata、participants 和 user:<id> tags，不基于记忆文本做业务关键词判断；userID 和 participants 必须兼容数组和逗号分隔字符串。
- recall query 必须包含当前用户原话、最近用户追问和目标 persona；轻量寒暄默认不召回；query planner 默认 auto，只整理指代和上下文，不扩展业务关键词。
