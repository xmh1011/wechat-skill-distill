...
6. [云部署指南](docs/DEPLOYMENT.md)：把后端部署到 Hugging Face Spaces，把静态前端部署到 Tiiny Host。
...
Tiiny Host 只能托管静态前端；完整聊天需要一个后端服务承载 `/api/runtime`、`/api/skills` 和 `/api/chat`。推荐用 Hugging Face Spaces 的免费 CPU Docker Space 部署仓库里的 `Dockerfile`，再把 Tiiny 前端通过 `?apiBase=https://your-space.hf.space` 指向后端。
...
- [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)：Hugging Face Spaces 后端 + Tiiny 静态前端部署。
