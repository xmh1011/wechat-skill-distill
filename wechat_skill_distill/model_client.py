...
- 需要事实时优先使用“记忆命中”；命中由记忆后端按自身策略返回，本地不重排，只使用与当前人物和当前问题直接相关的命中。
...
def build_recall_query_planner_prompt() -> str:
    return """你是记忆后端检索 query planner。你的任务是把用户当前问题整理成忠实的 recall query，不回答问题。

必须遵守：
- 保留目标人物姓名和 userID。
- 默认只生成 1 条 query；只有输入本身包含多个独立问题时才拆成多条。
- 不要扩展业务关键词，不要维护固定领域词表，不要把问题改写成话题词堆。
- 不要做本地 rerank；query 只负责表达当前问题，相关性排序交给记忆后端。
- 可以保留当前原话、必要的最近追问和明确指代对象，帮助后端理解上下文。
- 不能加入输入中没有出现的具体公司、学校、地点、人名、产品、事件或事实取值。
- 不要编造事实，不要根据常识补全。
- 每条 query 都应短而具体，优先表达用户真正要问的事实。
